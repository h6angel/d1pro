...
本文描述本仓库在 **Gazebo + D1 地面机器人** 场景下的整体数据流：感知建图、规划状态机、轨迹采样与底盘控制。数学细节见：
...
EGO Planner 原为四旋翼 3D 避障规划器。本仓库将其适配为 **固定高度平面运动** 的差速底盘：规划在 XY 平面进行，高度 $z$ 锁定为当前 `/odom` 高度；执行层通过 `traj_server` 与 `d1_planner_bridge` 将 B 样条轨迹转为 `cmd_vel`。

```mermaid
flowchart TB
  subgraph perception["感知 / 建图"]
    Odom["/odom"]
    Cloud["/gazebo_obstacles"]
    GM["GridMap 体素占据"]
    Odom --> GM
    Cloud --> GM
  end
...
| 话题 | 类型 | 生产者 | 消费者 | 说明 |
|------|------|--------|--------|------|
| `/odom` | `nav_msgs/Odometry` | 仿真/机器人 | 规划、traj_server、bridge | 位姿与速度闭环 |
| `/gazebo_obstacles` | `sensor_msgs/PointCloud2` | 仿真 | `GridMap` | 障碍点云直接标占据 |
...
仿真环境：[d1sim](https://github.com/h6angel/d1sim)。
...
| 数据 | 话题（D1 默认） | 作用 |
|------|-----------------|------|
| 里程计 | `/odom` | 机器人世界系位姿；划定局部地图更新范围 |
| 障碍点云 | `/gazebo_obstacles` | 点云落入局部区域 → 占据体素 + 膨胀 |

### 2.2 处理流程

1. `/odom` 更新相机/机体位置，确定以机器人为中心的局部更新窗口。
2. `cloudCallback` 将点云中各点（及膨胀邻域）写入 `occupancy_buffer_inflate_`。
...
- `flight_type=2`：`single_run_d1.launch.py` 中预设航点

### 3.4 重规划与到达判定（相对原版的重要改动）

| 机制 | 行为 |
|------|------|
| **首次规划** | `planFromGlobalTraj`：`flag_polyInit=true`，从 `/odom` 到局部目标生成多项式初值 |
...
| 文件 | 内容 |
|------|------|
| `src/planner/plan_manage/launch/single_run_d1.launch.py` | D1 地图、速度、重规划周期、traj_server |
