...
| `GEN_NEW_TRAJ` | 从全局路径生成/恢复轨迹（首次规划、急停恢复、手动 TRIG） |
...
1. `enterEmergencyStop()` 进入 `EMERGENCY_STOP` 并调用 `callEmergencyStop()`，发布 **6 个重合控制点** 的停车 B 样条（`n=6`）。
2. 当 `odom_vel_.norm() < 0.1` 且 `fsm/fail_safe=true` 时，置 `pending_estop_global_replan_=true` 并转入 `GEN_NEW_TRAJ`。
3. `GEN_NEW_TRAJ` 入口若 `pending_estop_global_replan_` 为真，先调用 `maybeReplanGlobalAfterEstop()`：当 odom 相对全局路径 XY 漂移超过 `fsm/global_replan_drift_thresh`（默认 0.25 m）时，以当前 odom 为起点重建 `global_data_`（log 标签 `[global_replan]`）。
4. 随后执行 `planFromGlobalTraj(10)` → `reboundReplan`。
5. `d1_planner_bridge` 在 `plan_vel < hard_stop_plan_speed`（默认 **0.005** m/s，`d1_bridge.yaml`）时强制 `cmd_vel = (0, 0)`，log 标签 `[cmd_vel_pub] hard_stop`。
...
| **局部目标选取** | `getLocalTarget()` 沿全局路径按 `planning_horizon` 切片；距离仍用 **3D `.norm()`**（非 XY），与到达判定不同 |
...
| 文件 | 内容 |
|------|------|
| `src/planner/plan_manage/config/d1_robot.yaml` | **话题、限速、相机内参、规划子集**（`map_size_*`、`planning_horizon`、`goal_reach_thresh`、`tag_stop_dist` 等） |
...
| [REMAINING_PRS.md](../REMAINING_PRS.md) | 急停恢复等待落地 PR 清单 |
