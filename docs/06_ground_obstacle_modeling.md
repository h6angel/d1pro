...
**状态：主体已落地，且实机高度/膨胀相关参数已调妥**（见 `d1_robot.yaml` / `grid_map`）。下文保留原理说明；日常不必再当「待标定」项。
...
当前 `camera_to_ground` / `obstacle_min_height` / 膨胀与 footprint **已按实机确认合适**。仅在更换相机安装高度、机身外形或场景尺度后，再回头改这些参数。

仍属算法/结构级、与日常标定无关的方向：

1. **方案 B（未做）**：建图时把柱压成 2D 占据层——目前靠查询侧柱检查即可。
2. **ESDF / 更平滑障碍梯度**：见 [todo.md](todo.md)。
